...
/*   Updated: 2020/07/23 17:18:29 by czhang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_thread_env(t_thread_env *thread_env)
{
	...
	ft_memcpy(thread_env->map.data, doom->map.data, sizeof(char)
			* (thread_env->map.nbcol * thread_env->map.nbl + 1));
	ft_memcpy(thread_env->map.bright, doom->map.bright, sizeof(int)
			* thread_env->map.nbcol * thread_env->map.nbl);
	ft_memcpy(thread_env->map.alt, doom->map.alt, sizeof(int)
			* thread_env->map.nbcol * thread_env->map.nbl);
...
void	init_thread_env(t_env *doom, int i)
{
	...
	new_thread_env->screen_pixels = (Uint32*)ft_memalloc(sizeof(Uint32) * W * H);
...
void	init_pthread(t_env *doom)
{
	...
	shared_data->tab_thread_env = (t_thread_env*)ft_memalloc(sizeof(t_thread_env) * shared_data->max_thread);
